---
title: "Omni123: Exploring 3D Native Foundation Models with Limited 3D Data by Unifying Text to 2D and 3D Generation"
authors:
  - Chongjie Ye
  - Cheng Cao
  - Chuanyu Pan
  - Yiming Hao
  - Yihao Zhi
  - Yuanming Hu
  - Xiaoguang Han
venue: arXiv preprint
venueZh: arXiv 预印本
venueShort: Preprint
venueShortZh: 预印本
summaryZh: 把文本、图像与三维视作同一个 token 序列，让充足的二维数据在三维数据稀缺处充当几何先验。
year: 2026
date: '2026-04-02'
thumb: /images/pub/omni123.jpg
featured: true
links:
  paper: https://arxiv.org/abs/2604.02289
---
Treats text, images and 3D as one token sequence, so plentiful 2D data acts as a geometric prior where 3D training data is scarce.
